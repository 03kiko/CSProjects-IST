...
int main(int argc, char* argv[]) {
	...
	if(pthread_sigmask(SIG_BLOCK, &mask, NULL) != 0) {
		...
		return 1; // this ensures the program doesn't terminate
	} // without letting use close files, etc...
	...
	while (1) {
		...
		switch (get_next(in_fd)) {
			case CMD_CREATE:
				...
				if ((result = ems_create(event_id, num_rows, num_columns)) != 0)
			...
			case CMD_RESERVE:
				...
				if ((result = ems_reserve(event_id, num_coords, xs, ys)) != 0)
			...
			case CMD_SHOW:
				...
				if ((result = ems_show(out_fd, event_id)) != 0)
					fprintf(stderr, "Failed to show event\n");
				break;

			case CMD_LIST_EVENTS:
				if ((result = ems_list_events(out_fd)) != 0)
			...
			case EOC:
				...
				close(out_fd);
				if (ems_quit() != 0) {
					fprintf(stderr, "Something went wrong while quitting\n");
					return 1;
				}
				return 0;
		}
		if (result == -1) {
			fprintf(stderr, "Server disconnected, closing client\n");
			close(in_fd);
			close(out_fd);
			close_client_pipes();
			return 1;
