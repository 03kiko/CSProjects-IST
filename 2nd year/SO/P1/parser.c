...
static pthread_mutex_t global_input_lock;

int global_input_lock_init() {
	return pthread_mutex_init(&global_input_lock, NULL) != 0;
}

int global_input_lock_destroy() {
	return pthread_mutex_destroy(&global_input_lock) != 0;
}

void global_input_lock_acquire() {
	handle_lock_failure(pthread_mutex_lock(&global_input_lock));
}

void global_input_lock_release() {
	handle_unlock_failure(pthread_mutex_unlock(&global_input_lock));
}
...
static int read_uint(int fd, unsigned int *value, char *next) {
	...
	char* end_ptr = NULL;
	unsigned long ul = strtoul(buf, &end_ptr, 10);
