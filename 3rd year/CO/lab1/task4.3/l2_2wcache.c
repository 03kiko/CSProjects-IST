...
uint8_t L2CacheBuffer[L2_SIZE];
CacheLine L2CacheLines[L2_SIZE / BLOCK_SIZE];
uint8_t L2MRUBuffer[L2_SIZE / (BLOCK_SIZE * 2)];

uint8_t L1CacheBuffer[L1_SIZE];
CacheLine L1CacheLines[L1_SIZE / BLOCK_SIZE];

MemoryLayer DRAM;
AssociativeCache L2Cache;
Cache L1Cache;
...
void initCache() {
	...
	L2Cache.cache.layer.buffer = L2CacheBuffer;
	L2Cache.cache.layer.blockCount =  L2_SIZE / BLOCK_SIZE;
	L2Cache.cache.layer.next = &DRAM;
	L2Cache.cache.layer.readTime = L2_READ_TIME;
	L2Cache.cache.layer.writeTime = L2_WRITE_TIME;
	L2Cache.cache.lines = L2CacheLines;
	L2Cache.cache.associative = 1;
	L2Cache.MRUbuffer = L2MRUBuffer;

	L1Cache.layer.buffer = L1CacheBuffer;
	L1Cache.layer.blockCount =  L1_SIZE / BLOCK_SIZE;
	L1Cache.layer.next = &L2Cache.cache.layer;
